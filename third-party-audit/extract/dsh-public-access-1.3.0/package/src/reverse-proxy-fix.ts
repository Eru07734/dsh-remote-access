...
function handleLogoutRoute(_req: any, res: any) {
  res.writeHead(302, {
...
export function apply(ctx: Context) {
  ...
  ctx.effect(() => webServer.register({
    kind: 'exact',
    path: '/_auth/login',
    handler: (req: any, res: any) => handleLoginRoute(req, res)
  }), 'reverse-proxy-fix: /_auth/login route')

  ctx.effect(() => webServer.register({
    kind: 'exact',
    path: '/_auth/logout',
    handler: (_req: any, res: any) => handleLogoutRoute(_req, res)
  ...
  const conn = ctx.get('connection') as any
